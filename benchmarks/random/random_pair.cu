...
void BM_weak_sequential_load(benchmark::State &state) {
  ...
  auto const end = thrust::make_counting_iterator<int32_t>(input.size());

  for (auto _ : state) {
    cuda_event_timer raii{state};
    thrust::for_each(thrust::device, begin, end,
                     [input_data = input.data().get()] __device__(auto index) {
                       // volatile auto first = input_data[index].first;
                       // volatile auto second = input_data[index].second;
                     });
  }
...
BENCHMARK_TEMPLATE(BM_weak_sequential_load, int32_t, int32_t)
    ->RangeMultiplier(10)
